...
public class UserPointingModule : ModuleBase
{
    ...
    protected override void Start()
    {
        ...
        PreventDivideByZeroFromConfiguration();
    ...
    private void ShowCoordinates()
    {
        ...
        if (this.isBodyMode)
        {
            ...
            if (this.isPointPosRightValid)
            {
                screenPos = this.pointPosRight;
                this.isPointPosRightValid = false;
            }
            else if (this.isPointPosLeftValid)
            {
                screenPos = this.pointPosLeft;
                this.isPointPosLeftValid = false;
            }
            else
            {
                SetValue("user:isPointing", true, "User doing pointing gesture");
                DataStore.ClearValue("user:pointPos");
                SetValue("user:pointValid", false, "Pointing position is not on screen");
                return;
            }
        }
        ...
        if (Physics.Raycast(ray, out hit, maxDistance, layerMask))
        {
			...
			if (!calibrationMode)
			{
				SetValue("user:isPointing:right", this.isPointPosRightValid, comment);
				SetValue("user:isPointing:left", this.isPointPosLeftValid, comment);
    ...
    private void GetPixelSpaceCoordinates()
    {
        ...
        if (leftPoint)
        {
            ...
            if (leftPointPos.x == -float.MaxValue || leftPointPos.y == -float.MaxValue)
                leftValid = false;
            else
                leftValid = true;
            ...
            else
            {
                ...
                if (!this.isPointPosLeftValid)
                {
                    leftPointList.Clear();
                    handTipLeftList.Clear();
                    shoulderLeftList.Clear();
                }
            }
        }
        else // if the user isn't pointing we want to clear lists so prior info isn't preserved
        {
            leftPointList.Clear();
            handTipLeftList.Clear();
            shoulderLeftList.Clear();
        ...
        if (rightPoint)
        {
            ...
            if (rightPointPos.x == -float.MaxValue || rightPointPos.y == -float.MaxValue)
                rightValid = false;
            else
                rightValid = true;
    ...
    private Vector3 AverageJoint(List<Vector3> jointList)
    {
        ...
        if (jointList.Count > 0)
        {
            ...
            return averageJointPos / jointList.Count;
        }
    ...
    {
        if (handTip.x == -float.MaxValue || shoulder.x == float.MaxValue)
            return new Vector3(-float.MaxValue, -float.MaxValue, -float.MaxValue);
    ...
    private void PreventDivideByZeroFromConfiguration()
    {
        if (this.rHKinSpaceTopRight.x == 0.0f)
            this.rHKinSpaceTopRight.x += 0.001f;
        if (this.rHKinSpaceTopRight.y == 0.0f)
            this.rHKinSpaceTopRight.y += 0.001f;
        if (this.rHKinSpaceBotLeft.x == 0.0f)
            this.rHKinSpaceBotLeft.x += 0.001f;
        if (this.rHKinSpaceBotLeft.y == 0.0f)
            this.rHKinSpaceBotLeft.y += 0.001f;

        if (this.lHKinSpaceTopRight.x == 0.0f)
            this.lHKinSpaceTopRight.x += 0.001f;
        if (this.lHKinSpaceTopRight.y == 0.0f)
            this.lHKinSpaceTopRight.y += 0.001f;
        if (this.lHKinSpaceBotLeft.x == 0.0f)
            this.lHKinSpaceBotLeft.x += 0.001f;
        if (this.lHKinSpaceBotLeft.y == 0.0f)
            this.lHKinSpaceBotLeft.y += 0.001f;
    }
}
