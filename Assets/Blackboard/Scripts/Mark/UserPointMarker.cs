...
public class UserPointMarker : MonoBehaviour
{
	...
	[Tooltip("Indicator color when me:isAttendingPointing is true")]
	public Color validAttendedColor = new Color(0.5f, 0, 1f);
	[Tooltip("Indicator color when me:isAttendingPointing is false")]
	public Color validUnattendedColor = new Color(0.5f, 0.5f, 0.5f);
	...
	protected void LateUpdate() {
		if (!DataStore.GetBoolValue("user:isPointing")) {
			...
			if (DataStore.GetBoolValue("user:pointValid")) {
				...
				if (DataStore.GetBoolValue("me:isAttendingPointing", true)) {
					renderer.color = validAttendedColor;
				} else {
					renderer.color = validUnattendedColor;
				}
			} else {
